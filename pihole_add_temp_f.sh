...
sed -i '/<span id="memory"><\/span>/s/<\/span>/<\/span><br\/>\n                    <span id="temperature">\&nbsp;\&nbsp;<i class="fa-solid fa-temperature-three-quarters text-green-light"><\/i>\&nbsp;\&nbsp; Temp: <%= string.format("%.1f°F", (tonumber(io.open("\/sys\/class\/thermal\/thermal_zone0\/temp"):read("*a")) \/ 1000) * 9\/5 + 32) %><\/span>/' "$SIDEBAR_FILE"
